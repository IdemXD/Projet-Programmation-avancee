	...
	while(jouer && !data->terminer)
	{
				...
				case SDL_KEYDOWN:
					switch(evenements.key.keysym.sym)
					{
						...
						case SDLK_DOWN:
								...
								if(data->etape == 2 && (data->joueur[data->tour_perso].actions[data->tour_action]>0  && data->joueur[data->tour_perso].actions[data->tour_action]<4 && data->move==0 )){ //Seulement si le joueur est en train de faire un choix de direction
						...
						case SDLK_UP:
							if(data->etape == 2 && (data->joueur[data->tour_perso].actions[data->tour_action]>0  && data->joueur[data->tour_perso].actions[data->tour_action]<4 && data->move==0 )){
		...
		if (data->active_direction!='n'){
			//On attend que le joueur choisisse une direction pour appliquer l'action 'contrôler' ou 'déplacer'
                applique_action(data->salles, data->joueur, &(data->active_direction), data->tour_action,data->tour_perso, data->nb_personnages,&(data->active_direction_salle),&(data->move));
